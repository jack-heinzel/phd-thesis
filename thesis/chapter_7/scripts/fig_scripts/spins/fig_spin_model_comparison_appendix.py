...
# Load inputs
fpaths = glob.glob('../../../analyses/matt/popsummary_filepath_BBHSpin_*.txt')

popsummary_results = {}
for fpath in fpaths: 

    if 'MagTruncnormIndTiltIsotropicTruncnormNnd' in fpath or 'MagTruncnormIidTiltIsotropicTruncnormNid' in fpath:

        with open(fpath, "r") as file:
            fname = file.read().split('\n')[0] # first line in the file    
            key = fpath.split('popsummary_filepath_BBHSpin_')[-1].split('.txt')[0]

            print(key)
    
            # load popsummary file
            popsummary_results[key] = PopulationResult(fname=fname)
...
i = 'MagTruncnormIndTiltIsotropicTruncnormNnd'
...
i = 'MagTruncnormIidTiltIsotropicTruncnormNid'
...
plt.savefig(pf.figs_folder+'spin_model_comparison_cornerplot.pdf', bbox_inches='tight')
plt.close()