...
with open(f'../../../analyses/jaxen/path_to_popsummary_files.txt', "r") as file:
    fname = file.read().split('\n')[0] # first line in the file
    result_bspline = PopulationResult(fname=fname)

# load the default data
with open(f'../../../analyses/matt/popsummary_filepath_BBHSpin_MagTruncnormIidTiltIsotropicTruncnormNid.txt', "r") as file:
    fname = file.read().split('\n')[0] # first line in the file
...
# save figure 
plt.savefig(pf.figs_folder+'spin_magnitudes_and_tilts.pdf', bbox_inches='tight')
