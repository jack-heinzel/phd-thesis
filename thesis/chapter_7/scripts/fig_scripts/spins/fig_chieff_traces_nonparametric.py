...
# load the data
popsummary_filepaths = {}
for analyst, fname in zip(
    ['anarya', 'sharan', 'jaxen', 'simona', 'jack'],
    ['popsummary_filepath_BBHCorr_MassSpinCorrelatedBGPModel', 'popsummary_filepath_BBHSpin_EpsSkewNormalChiEff', 
     'path_to_chi_popsummary_file', 'popsummary_filepath_BBHSpin_GaussianChiEffChiP', 'popsummary_filepath_BBHCorr_qchieffSplineCorrelationModel']
): 
    with open(f'../../../analyses/{analyst}/{fname}.txt', "r") as file:
        popsummary_filepaths[analyst] = file.read()

bgp_chieff = PopulationResult(fname=popsummary_filepaths['anarya'])
skewnormal_chieff = PopulationResult(fname=popsummary_filepaths['sharan'].strip('\n'))
bspline_chieff =  PopulationResult(fname=popsummary_filepaths['jaxen'])
chieff_chip = PopulationResult(fname=popsummary_filepaths['simona'])
chieff_q_corr = PopulationResult(fname=popsummary_filepaths['jack'])
...
for k,v in plotting_dict.items(): 
    ...
    print(f'HM fraction for {k} model', np.quantile(10**samples[~mask], (0.1,0.5,0.9)))
...
plt.savefig(pf.figs_folder+'chi_eff_traces_nonparametric.pdf', bbox_inches='tight')
plt.close()