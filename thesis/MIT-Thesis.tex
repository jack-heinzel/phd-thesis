...
%% \DocumentMetadata is disabled: even the minimal form activates LaTeX3 environment
%% tracking which conflicts with \beq/\eeq macros inside biblatex refsections.
%% pdfcontact* keys in mitthesis.cls are guarded with \@ifundefined so they degrade
%% gracefully when \DocumentMetadata is not loaded.
%\DocumentMetadata
%{
%	lang       = en-US,
%	pdfversion = 1.7,
%	pdfstandard = a-2b,   % disabled: conflicts with \beq/\eeq in refsections
%	tagging     = on,     % disabled: requires lualatex + no listings package
%	debug       = { xmp-export },
%}
...
% hyperxmp and hyperref are already loaded by mitthesis.cls; loading them here
% late (after hyperref) causes interaction issues with \DocumentMetadata.
...
\usepackage{siunitx}         % \SI{}{} units formatting (chapter 7)
\usepackage{textgreek}       % text-mode Greek letters (chapter 7)
\usepackage{scrextend}       % KOMA footnote extensions (chapter 7)
\usepackage{makerobust}      % \MakeRobust command (chapter 7 macros)
...
\graphicspath{
  ...
  {thesis/chapter_7/}
  {thesis/chapter_7/figures/}
  {thesis/chapter_7/figures/mass/}
  {thesis/chapter_7/figures/spins/}
}
...
\addbibresource{thesis/introduction/introduction.bib}
...
\addbibresource{thesis/chapter_7/references.bib}
\addbibresource{thesis/chapter_7/gwtc-common-files/references/base_bibliography.bib}
\addbibresource{thesis/chapter_7/gwtc-common-files/references/bibliography.bib}
\addbibresource{thesis/chapter_7/gwtc-common-files/references/compendia.bib}
...
%% Chapter 7 (GWTC-4) macros — loaded here so every macro is available when
%% .lof/.lot/.toc aux files are read back at the start of the document.
%% Safety patches: \usepackage → no-op, \newcommand → \providecommand.
\newcommand{\gwtccommonfiles}{thesis/chapter_7/gwtc-common-files}
\makeatletter
\input{thesis/chapter_7/macros}
\input{thesis/chapter_7/macros_from_json}
\input{\gwtccommonfiles/commands}
\input{\gwtccommonfiles/macros/symbol_macros}
\input{\gwtccommonfiles/macros/misc_macros}
\input{\gwtccommonfiles/macros/datetime_macros}
\input{\gwtccommonfiles/macros/quantities_macros}
\input{\gwtccommonfiles/acronyms}
...
%% Author list and affiliations for journal-paper chapters.
%% Usage:
%%   \chapterauthors{Author One$^1$, Author Two$^{1,2}$, Author Three$^2$}
%%   \chapteraffiliations{$^1$MIT, $^2$Some Other Institution}
\newcommand{\chapterauthors}[1]{%
  \medskip\noindent{#1}\par%
}
\newcommand{\chapteraffiliations}[1]{%
  \smallskip\noindent{\itshape #1}\par\medskip%
}
...
\providecommand{\variancelarge}[1]{\ensuremath{\mathbb{V}\left[#1\right]}}
...
%% AASTeX deluxetable compatibility shim (Chapter 7 uses AAS journal-style tables)
%% Provides: deluxetable, deluxetable*, \tablecaption, \tablehead, \colhead,
%%           \startdata, \enddata, \tablenotetext, \tablewidth, \tablecolumns
%% Note: supertabular pre-defines \tablecaption and \tablehead — we redefine to
%%       AASTeX semantics. Storage uses public names (no @) for access outside \makeatletter.
\def\dxtblhead{}   % stores column header row(s)
\def\dxtblcols{}   % stores column spec string
\AtBeginDocument{%
  \renewcommand{\tablecaption}[1]{\caption{#1}}%
  \renewcommand{\tablehead}[1]{\def\dxtblhead{#1}}%
}
\providecommand{\colhead}[1]{\textbf{#1}}
\providecommand{\colrule}{\midrule}
\providecommand{\tablewidth}[1]{}
\providecommand{\tablecolumns}[1]{}
\providecommand{\tableheadfrac}[1]{}
\providecommand{\tablenotetext}[2]{\vspace{4pt}\par{\small\textit{Note.}~#2}}
\providecommand{\tablecomments}[1]{\vspace{4pt}\par{\small\textit{Note.}~#1}}
\providecommand{\startdata}{%
  % Force expansion of \dxtblcols before passing to tabular column spec
  \edef\gwtcdx@tabbegin{\noexpand\begin{tabular}{\dxtblcols}}%
  \gwtcdx@tabbegin\toprule
  \dxtblhead\\
  \midrule
}
\def\enddata{\bottomrule\end{tabular}}
\newenvironment{deluxetable}[1]{%
  \def\dxtblhead{}\def\dxtblcols{#1}%
...
\makeatother % close the \makeatletter opened at the Chapter 7 macro-loading block above
...
%
\Reader{Erin Kara}{Associate Professor of Physics}{Department of Physics
\\ & MIT Kavli Institute for Astrophysics and Space Research}
\Reader{William Detmold}{Professor of Physics}{Department of Physics 
...
\include{thesis/chapter_7/paper}
