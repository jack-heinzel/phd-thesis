...
%% Minimal DocumentMetadata: registers PDF infrastructure so hyperxmp contact-info keys
%% (pdfcontactaddress, etc.) are defined by the mitthesis class without error.
%% tagging/pdfstandard accessibility options are intentionally omitted -- they trigger
%% LaTeX3 environment tracking that conflicts with \beq/\eeq inside biblatex refsections.
\DocumentMetadata
{
	lang       = en-US,
	pdfversion = 1.7,
