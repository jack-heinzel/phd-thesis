...
vt_redshift_model = gwpopulation.models.redshift.PowerLawRedshift()
...
# converted, added_keys = jit_likelihood.conversion_function(maxlike)
...
# for key in ['alpha', 'beta', 'mu', 'sigma']:
#     jit_likelihood.parameters.pop(key+'_chi')
#     jit_likelihood.hyper_prior.parameters.pop(key+'_chi')
#     jit_likelihood.selection_function.model.parameters.pop(key+'_chi')

# del like
# del jit_likelihood

# like = CorrectedPlusDagNabbitCustomLikelihood(posteriors, model, selection_function=selection_effects, maximum_uncertainty=maximum_uncertainty, max_samples=NPE, conversion_function=gwpopulation.conversions.convert_to_beta_parameters, posterior_length_corrections=posterior_length_corrections)#, fid_parameters=maxlike, reject_zscore=10) # reject more aggressively
# jit_likelihood = gwpopulation.experimental.jax.JittedLikelihood(like)
...
truths['likelihood_correction'] = None
truths['total_correction'] = None
