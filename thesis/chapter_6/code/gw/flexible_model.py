...
if opts.salvos_PE:
    run_label = 'flexible_models_salvo_pe/' + run_label
else:
...
for n in range(opts.nodes-1):
    priors[f'fa{n}'] = bilby.core.prior.conditional.DirichletElement(n, opts.nodes, label='fa')
priors.pop(f'fa{opts.nodes - 1}')

maxlike = true_hyperpar.copy()

truths = maxlike.copy()
fa_last = truths.pop(f'fa{opts.nodes-1}')
...
spin_model = flexible_spin_mag_model(opts.nodes, exp=not opts.no_exp, expm1=opts.expm1)
vt_spin_model = flexible_spin_mag_model(opts.nodes, exp=not opts.no_exp, expm1=opts.expm1)
...
# for ii in range(opts.nodes-1):
#     like_par[f'fa{ii}'] = 1 / opts.nodes + 0.005 * ii
# print(like_par)
...
truths[f'fa{opts.nodes-1}'] = fa_last
posterior = result.posterior
posterior[f'fa{opts.nodes-1}'] = np.ones(len(posterior)) - np.sum([posterior[f'fa{ii}'] for ii in range(opts.nodes - 1)], axis=0)
fs_keys = spin_model.variable_names
fs_keys.append(f'fa{opts.nodes-1}')

for key in fs_keys:
    posterior[key] = spin_model.scale(posterior[key])
