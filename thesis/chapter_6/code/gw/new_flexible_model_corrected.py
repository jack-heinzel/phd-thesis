...
def ParseCommandLine():
    ...
    parser.add_option('--result_nlive', type=int, default=0)
...
print('Loading original, uncorrected posterior...')
if opts.result_nlive >= 10:
    nlivestr = ''
    if opts.result_nlive != 1000:
        nlivestr = f"_nlive{opts.result_nlive}"
    ORIGINAL_RESULT_FILE = os.path.join(run_directory, 'sample_posterior' + nlivestr + '_result.json')
    original_result = bilby.core.result.read_in_result(ORIGINAL_RESULT_FILE)
...
old_posterior = original_result.posterior
print(f'...done.\nWe have {old_posterior.shape[0]} samples from the original posterior')
...
spin_model = flexible_spin_mag_model(opts.nodes)
vt_spin_model = flexible_spin_mag_model(opts.nodes)
...
selection_effects = CorrectedResamplingVT(vt_model, gwpop_data, old_posterior, n_events=opts.Nevents)
...
like = CorrectedPlusDagNabbitCustomLikelihood(posteriors, model, old_posterior, selection_function=selection_effects, minimum_neff=opts.MinimumNeff, maximum_uncertainty=maximum_uncertainty, max_samples=NPE, conversion_function=gwpopulation.conversions.convert_to_beta_parameters, posterior_length_corrections=posterior_length_corrections)#, fid_parameters=maxlike, reject_zscore=10) # reject more aggressively
jit_likelihood = gwpopulation.experimental.jax.JittedLikelihood(like)

nlivestr = ''
if opts.nlive != 1000:
    nlivestr = f"_nlive{opts.nlive}"
RESULT_FILE = os.path.join(run_directory, 'corrected_sample_posterior' + nlivestr + '_result.json')
...
if not os.path.exists(RESULT_FILE):
    result = bilby.run_sampler(
        likelihood=jit_likelihood, priors=priors, sampler='dynesty', label='corrected_sample_posterior'+nlivestr,
...
if opts.pin_extra_params:
    for p in priors:
        if (isinstance(priors[p], float) or isinstance(priors[p], int)) and p in truths:
            truths.pop(p)
