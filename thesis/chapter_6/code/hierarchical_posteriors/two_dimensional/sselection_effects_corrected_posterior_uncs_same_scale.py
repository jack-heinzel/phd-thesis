...
nvtlist = [10_000, 50_000, 100_000, 500_000, 1_000_000]
if VT_selection == 3:
    nvtlist = [5_000_000]
...
for nvt in nvtlist:
    ...
    # if Nobs == 1000:
    if VT_selection == 3:
        sigmas = jnp.linspace(-1,0.4,51)
        mus = jnp.linspace(-1,3,51)

        sigma_analytic = jnp.linspace(-1,0.4,301)
        mu_analytic = jnp.linspace(-1,3,301)
    elif VT_selection == 1:
        sigmas = jnp.linspace(-0.3,0.3,51)
        mus = jnp.linspace(0,2,51)

        sigma_analytic = jnp.linspace(-0.3,0.3,301)
