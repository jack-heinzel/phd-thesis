...
def random_posterior(PRNGkey, mu, sigma, observations, npe, noise_sigma, dmu, dsigma, big=False):
    ...
    return log_posterior, log_evidence, corrected_posterior - corr_log_evidence, corr_log_evidence, vs, wneff, correction
...
for npe in npelist:
    ...
    sigmas = jnp.linspace(-0.4,0.2,101)
    mus = jnp.linspace(0.55,1.45,81)
    ...
    def kl_and_ptrue(PRNGkey):
        ...
        log_posterior, log_evidence, clog_posterior, clog_evidence, vs, wneff, int_cov = random_posterior(PRNGkey, mu_mesh, sig_mesh, obs, npe, noise, dmu, dsigma, big=big)
        ...
        return KL_div / jnp.log(2), cKL_div / jnp.log(2), log_posterior, clog_posterior, analytic_log_posterior, (exp_v - exp_cov) / 2 / jnp.log(2), mean_sq_cov / 2 / jnp.log(2)
    ...
    for key in klist:
        k, e, p, pv, pw, prec, accu = kl_and_ptrue(key)
        kl.append(k)
        ckl.append(e)
        pest.append(p)
        cpest.append(pv)
