...
npelist = [10,20,50,100,200,500,1000,2000,10000]
npelist = [200,500,1000,2000,10000]
print(npelist)
for npe in npelist:
    ...
    sigmas = jnp.linspace(-1.5,0.5,101)
    ...
    os.makedirs(f'corrected_data_smaller/Ndim{ndim}_N{Nobs}_posteriors/', exist_ok=True)
    with open(f'corrected_data_smaller/Ndim{ndim}_N{Nobs}_posteriors/npe_{npe}.pkl', 'wb') as ff:
        pkl.dump((np.array(kl), np.array(ckl), np.array(pest), np.array(cpest), np.array(pw), np.array(mu_mesh), np.array(sig_mesh)), ff)