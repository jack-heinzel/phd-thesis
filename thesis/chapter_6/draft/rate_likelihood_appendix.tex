...
The likelihood may be estimated using Eqs.~\ref{eq: single event estimator} and \ref{eq: selection effects estimator}, but as pointed out in Sec.~\ref{subsec: mc monte carlo bias}, this represents a biased estimator. 
...
w_n = 
\begin{cases}
\dfrac{N_n^2\hat{\sigma}^2_{\xi}(\Lambda)}{2} - \dfrac{1}{N_{\rm samp}}\displaystyle\sum_{m=1}^{N_{\rm samp}} \hat{C}_{\ln\mathcal{L}}(\Lambda_n, N_n, \Lambda_m, N_m) & \begin{tabular}{@{}l@{}}  \text{if using uncorrected} \\ \text{likelihood (Eq.~\ref{eq: rate explicit likelihood})}\end{tabular} \\ 
\dfrac{1}{N_{\rm samp}}\displaystyle\sum_{m=1}^{N_{\rm samp}} \hat{C}_{\ln\mathcal{L}}(\Lambda_n, N_n, \Lambda_m, N_m)  & \begin{tabular}{@{}l@{}} \text{if using corrected} \\ \text{likelihood (Eq.~\ref{eq: rate likelihood correction}).}\end{tabular}
\end{cases}
\eeq
