...
\beq\begin{split}
    \hat{\Pi}[\hat{p}] &= \frac{1}{2\ln(2)}\bigg[\bigg(\frac{\Nobs^2}{\Nsamp}\sum_{n=1}^{\Nsamp}\sigma^2_{\ln \xi}(\Lambda_n) - \bar{K}_{\ln\xi}(\Lambda_n)\bigg) + \\ &\qquad \sum_{i=1}^{\Nobs}\bigg(\frac{1}{\Nsamp}\sum_{n=1}^{\Nsamp}\sigma^2_{\ln p_i}(\Lambda_n) - \bar{K}_{\ln p_i}(\Lambda_n)\bigg)\bigg]
...
\begin{cases}
\dfrac{\Nobs(\Nobs+1)\hat{\sigma}^2_{\xi}(\Lambda_n)}{2\hat{\xi}(\Lambda_n)^2} - \Nobs^2\bar{K}_{\ln \xi}(\Lambda_n) - \displaystyle\sum_{i=1}^{\Nobs} \bar{K}_{\ln p_i}(\Lambda_n) & \begin{tabular}{@{}l@{}}  \text{if using uncorrected} \\ \text{likelihood (Eq.~\ref{eq: estimated hierarchical likelihood})}\end{tabular} \\ 
...
\begin{align}
g(\hat{\bm X}(\Lambda)) &\to g\left(\left\{\frac{1}{M_j}\sum_{i=1}^{M_j}\hat{W}_{ij}^\Lambda\right\}_{j=1}^{N}\right)m\left(\left\{\hat{W}_{ij}^\Lambda\right\}_{i=1,j=1}^{M_j,N}, \left\{\hat{W}_{ij}^{\Lambda'}\right\}_{i=1,j=1}^{M_j,N}\right) \nonumber \\ &\equiv g(\hat{\bm W}^\Lambda)m(\hat{\bm W}^\Lambda, \hat{\bm W}^{\Lambda'}).
...
\begin{align}
m(\hat{\bm W}^\Lambda, \hat{\bm W}^{\Lambda'}) = \exp\Bigg(\sum_{p,q=1}^{M_q,N}\Bigg[&-\frac{1}{2}\frac{\partial^2g(\hat{\bm W}^\Lambda)}{(\partial \hat{W}^\Lambda_{pq})^2}\frac{\hat{C}^2_q(\hat{\bm W}^\Lambda, \hat{\bm W}^\Lambda)}{g(\hat{\bm W}^\Lambda)} \nonumber \\ &+ \int {\rm d}\Lambda' \hat{p}(\Lambda') \frac{\partial \ln g(\hat{\bm W}^\Lambda)}{\partial \hat{W}^\Lambda_{pq}}\frac{\partial \ln g(\hat{\bm W}^{\Lambda'})}{\partial \hat{W}^{\Lambda'}_{pq}}\hat{C}^2_q(\hat{\bm W}^\Lambda, \hat{\bm W}^{\Lambda'})\Bigg]\Bigg)
\label{eq: modifying term}
\end{align}
where 
\beq
...
\beq
g(\hat{\bm W}^{\Lambda})m(\hat{\bm W}^\Lambda, \hat{\bm W}^{\Lambda'})
\eeq
