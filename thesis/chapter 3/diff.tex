\documentclass[%DIF LATEXDIFF DIFFERENCE FILE
reprint,
 amsmath,amssymb,
 aps,
...
%DIF -------
\newcommand{\red}[1]{{\color{Red} #1}} %DIF > 
%DIF -------
...
\DIFdelbegin \DIFdel{In a companion paper \mbox{%DIFAUXCMD
\cite{Heinzel:2024hva}}\hskip0pt%DIFAUXCMD
, we analyze in detail possible parameter correlations in the population of binary BH mergers using public LVK data from the third GW transient catalog (GWTC-3).
}\DIFdelend 

The remainder of the paper is structured as follows. In Sec.~\ref{sec: Methods} we describe the Bayesian population inference problem and our procedure for estimating the likelihood and handling its uncertainty. In Sec.~\ref{sec: car model} we describe \textsc{PixelPop}. We discuss the importance of using minimal smoothing for astrophysical populations and its computational advantages. In Sec.~\ref{sec: simulated populations} we validate our method on synthetic populations. Finally, we conclude and point to further applications of \textsc{PixelPop} in Sec. \ref{sec:conclusion}. \DIFaddbegin \DIFadd{In a companion paper \mbox{%DIFAUXCMD
\cite{Heinzel:2024hva}}\hskip0pt%DIFAUXCMD
, we analyze in detail possible parameter correlations in the population of binary BH mergers using public LVK data from the third GW transient catalog (GWTC-3).
We make available the data for both of these papers at Ref.~\mbox{%DIFAUXCMD
\cite{heinzel_2024_13176116}}\hskip0pt%DIFAUXCMD
.
}\DIFaddend 
...
\mathcal{R} ( \theta' ; z )
=
\frac{ \dd{N} }{ \dd{V_\mathrm{c}} \dd{t_\mathrm{s}} \dd{\theta'} }
=
...
=
\sum_{n=1}^{N_\mathrm{obs}}
\frac{ \hat{\sigma}_n^2 }{ \hat{\mathcal{L}}_n[R]^2 }
+
...
\frac{ \kappa ( \ln\mathcal{R}_\beta - \ln\mathcal{R}_b )^2 + ( 1 - \kappa ) ( \ln\mathcal{R}_\beta - \mu )^2 }{ 2 \sigma^2 }
]
\nonumber
\, .
\end{align}
...
=
\sqrt{
\frac{ \det ( \mathbf{D} - \kappa \mathbf{A} ) }{ (2\pi \sigma^2 )^B }
}
\nonumber \\
& \times
\mathrm{exp}
...
\DIFaddbegin \DIFadd{In general, we find that $\kappa$ favors values $\approx1$. In this limit, the merger-rate bins do not couple to the global mean $\mu$ and as such $\mu$ does not affect the likelihood and sampling it just returns the prior; see Eq.~(\ref{eq: car example}). The variance $\sigma$ in the values of the merger-rate density is constrained away from the prior, however.
}\DIFaddend 
...
p(\theta)
=
\frac{ \mathcal{R}(\theta) }{ \int \dd{\theta'} \mathcal{R}(\theta') }
=
\frac{ \mathcal{R}(\theta) }{ \delta\theta \sum_{b=1}^B \mathcal{R}(\theta_b) }
\propto
\mathcal{R}(\theta)
\, .
...
\log_2 \frac{ \mathcal{P}(\mathcal{R}_\beta) }{ \pi(\mathcal{R}_\beta) }
\, ,
\label{eq: kl}
\end{align}
...
We create an additional set of software injections for computing Eq.~(\ref{eq: Nexp estimator}). For the reference distribution $p(\theta|\mathrm{inj})$ we use the same mass and redshift distributions as the true population (see Tab.~\ref{tab:true_hyper}). We draw spins uniform in magnitude and isotropic in direction. All remaining source parameters follow the uninformative PE priors. We draw a total of $5\times10^8$ sources, $10^6$ of which are detectable according to our SNR criterion.
\DIFaddbegin \DIFadd{Note that we do not perform PE for these sources as we only require computing the SNR to determine detectability.
}\DIFaddend 
...
\begin{figure*}
\centering
\DIFaddbeginFL \includegraphics[width=0.42\linewidth]{ml}
\DIFaddFL{\hspace{6mm}
...
Future work can use \textsc{PixelPop} to infer populations in even higher dimensions or just in a single dimension---the method described in Sec.~\ref{sec: Conditional autoregressive prior} applies to any number of dimensions, in principle. In preliminary tests we found that three dimensions with 50 bins per dimension is computationally feasible, i.e., $>10^5$ total parameters in the posterior of Eq.~(\ref{eq: posterior}). However, the HMC sampling time becomes slower and we are inevitably subject to the curse of dimensionality.
\DIFaddbegin \DIFadd{It may be possible to gain computational efficiency via sparse matrix representations and limiting }\textsc{\DIFadd{PixelPop}} \DIFadd{to an intrinsic CAR model \mbox{%DIFAUXCMD
\cite{MORRIS2019100301}}\hskip0pt%DIFAUXCMD
.
}\DIFaddend An approach to overcome the large number of parameters may be to adaptively choose bin locations and resolution during inference based on regions of higher information gain, reducing the number of bins placed in regions of parameter space where the inferred merger rate posteriors are broad.
...
Nonetheless, we have demonstrated the efficacy of \textsc{PixelPop} in inferring the underlying multidimensional merger rate of GW populations that have nontrivial correlations between source parameters---with high resolution, computational efficiency, and minimal model assumptions. As the number of GW observations increases, nonparametric methods will offer increasingly useful flexibility\DIFaddbegin \DIFadd{.
A data release containing the results of our }\textsc{\DIFadd{PixelPop}} \DIFadd{analysis is available in Ref.~\mbox{%DIFAUXCMD
\cite{heinzel_2024_13176116}}\hskip0pt%DIFAUXCMD
}\DIFaddend .
